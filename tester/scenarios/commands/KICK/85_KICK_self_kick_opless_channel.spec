...
C2 SEND JOIN #lobby
C2 EXPECT :Bob140!* JOIN #lobby
C1 WAIT_RECV :Bob140!* JOIN #lobby

# Alice kicks herself from #lobby
C1 SEND KICK #lobby Ali140 :Goodbye all
C1 EXPECT :Ali140!* KICK #lobby Ali140 :Goodbye all
C2 EXPECT :Ali140!* KICK #lobby Ali140 :Goodbye all

# Bob tries to set mode or kick, but is rejected because channel has no remaining ops
C2 SEND MODE #lobby +i
C2 EXPECT 482 Bob140 #lobby :You're not channel operator
C2 SEND KICK #lobby Ali140 :not op
C2 EXPECT 482 Bob140 #lobby :You're not channel operator
