...
{
	//Client socket flags gets set to nonblocking, otherwise the socket gets closed.
	...
	//Adds the client socket to the fds. And the event to listen for gets set to POLLIN.
...
void	Server::server_loop()
{
	...
	while (true)
	{
		...
		{
			//If the current fd doesn't have POLLIN (pending input) set, go to the next one.
			if (!(get_fds()[index].revents & POLLIN))
				continue;

			//If the fd has POLLIN set and is the server_socket, it means a client wants to connect.
			if (get_fds()[index].fd == get_server_socket())
			{
				//Client gets accepted and gets a own socket, connected to the server socket.
